...
import { z } from "zod";
import { factualityModel } from "../app/api/chat/model";
...
export const AnswerRelevancy = createScorer<string, string, string>({
  ...
  scorer: async ({ input, expected, output }) => {
    // input: user question, expected: ground truth, output: model output
    const statements = await generateStatements(output);
    const { verdicts } = await scoreRelevancy(input, statements);
    // Map verdicts to scores
    const scoreMap = { yes: 1, unsure: 0.5, no: 0 };
    const scores = verdicts.map((v) => scoreMap[v.verdict]);
    const score =
